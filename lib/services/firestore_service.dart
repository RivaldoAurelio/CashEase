...
class FirestoreService {
  ...
  // ==================== SECURITY METHODS (BARU) ====================

  // Update PIN User di Firestore
  Future<bool> updatePin(String phone, String newPin) async {
    try {
      // 1. Cek apakah user ada
      final user = await getUserByPhone(phone);
      if (user == null) {
        print('❌ User not found for PIN update');
        return false;
      }
      ...
      // 3. Update field 'pin' di dokumen user
      await _db.collection(_colUsers).doc(phone).update({
        'pin': hashedPin,
        'updated_at': DateTime.now().toIso8601String(),
      });
      
      print('✅ PIN updated successfully for: $phone');
      return true;
    } catch (e) {
